...
#### 3. 네트워크 실시간 속도(KB/s) 측정 한계 극복
- **문제:** 안드로이드 시스템은 앱의 현재 네트워크 속도를 직접 제공하지 않고, 기기 부팅 후 누적된 전체 바이트(Bytes) 사용량만 제공하여 실시간 패킷 증감폭을 파악하기 어려움.
- **해결:** `dumpsys package`를 통해 앱의 고유 UID를 추출한 뒤, `/proc/net/xt_qtaguid/stats` 시스템 파일에서 해당 UID의 수/송신 누적 바이트를 파싱. 이후 프론트엔드 상태(`useRef`)에 이전 1초의 누적값을 저장하고, 현재 누적값과의 차이(Delta)를 계산하여 '실시간 초당 전송 속도(KB/s)'로 변환하는 알고리즘 구현.
